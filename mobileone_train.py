...
from dataset.brainstorm import (
    BraDataset
)
...
class Trainer:
    ...
    def parse_args(self):
        ...
        # Model
        parser.add_argument('--model-variant', type=str, default='mobileone',
                            required=False, choices=['mobileone'])
        # Matting dataset
        parser.add_argument('--dataset', type=str, default='videomatte', required=False,
                            choices=['videomatte', 'imagematte', 'brainstorm'])
        # Learning rate
        ...
        parser.add_argument('--log-dir', type=str, required=False, default='Experiments/MobileOne-S0/exp_00/stage1/tensorboard')
        parser.add_argument('--log-train-loss-interval', type=int, default=100)
        parser.add_argument('--log-train-images-interval', type=int, default=100)
        # Checkpoint loading and saving
        parser.add_argument('--checkpoint', type=str, required=False)
        parser.add_argument('--checkpoint-dir', type=str, required=False, default='Experiments/MobileOne-S0/exp_00/stage1/checkpoint')
        parser.add_argument('--checkpoint-save-interval', type=int, default=100)
        # Distributed
    ...
    def init_datasets(self):
        ...
        if self.args.dataset == 'videomatte':
        ...
        elif self.args.dataset == 'brainstorm':
            self.dataset_lr_train = BraDataset(
                videomatte_dir=DATA_PATHS['brainstorm']['train'],  # videomatte
                background_image_dir=DATA_PATHS['background_images']['train'],
            ...
            if self.args.train_hr:
                self.dataset_hr_train = BraDataset(
                    videomatte_dir=DATA_PATHS['brainstorm']['train'],  # videomatte
                    background_image_dir=DATA_PATHS['background_images']['train'],
                    background_video_dir=DATA_PATHS['brainstorm_bgs']['train'],
                    size=self.args.resolution_hr,
            ...
            self.dataset_valid = BraDataset(
                videomatte_dir=DATA_PATHS['brainstorm']['valid'],
                background_image_dir=DATA_PATHS['background_images']['valid'],
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            for true_fgr, true_pha, true_bgr in tqdm(self.dataloader_lr_train, disable=self.args.disable_progress_bar, dynamic_ncols=True):
                ...
                if self.step % self.args.checkpoint_save_interval == 0 and self.step != 1:
                    self.validate()
                    self.save()
    ...
    def validate(self):
        ...
        if self.rank == 0:
            self.writer.add_image(f'val_pred_fgr', make_grid(pred_fgr.flatten(0, 1), nrow=pred_fgr.size(1)), self.step)
            self.writer.add_image(f'val_pred_pha', make_grid(pred_pha.flatten(0, 1), nrow=pred_pha.size(1)), self.step)
            self.writer.add_image(f'val_true_fgr', make_grid(true_fgr.flatten(0, 1), nrow=true_fgr.size(1)), self.step)
            self.writer.add_image(f'val_true_pha', make_grid(true_pha.flatten(0, 1), nrow=true_pha.size(1)), self.step)
            self.writer.add_image(f'val_true_src', make_grid(true_src.flatten(0, 1), nrow=true_src.size(1)), self.step)
