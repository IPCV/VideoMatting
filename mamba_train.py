...
python train.py \
    ...
    --learning-rate-refiner 0 \
    --checkpoint Experiments/MobileOne-S0/Segmentation_Matting/exps0_01/stage1/epoch-9.pth \
    --checkpoint-dir checkpoint/exps0_01/stage2_1 \
    --log-dir log/stage2 \
    --epoch-start 10 \
    --epoch-end 15
...
class Trainer:
    ...
    def parse_args(self):
        ...
        # Model
        parser.add_argument('--model-variant', type=str, default='mamba',
                            required=False, choices=['mamba'])
        # Matting dataset
        parser.add_argument('--dataset', type=str, default='videomatte', required=False,
                            choices=['videomatte', 'imagematte', 'am2k'])
        # Learning rate
        parser.add_argument('--learning-rate-backbone', type=float, default=0.0001, required=False)
        parser.add_argument('--learning-rate-aspp', type=float, required=False, default=0.0002)
        parser.add_argument('--learning-rate-decoder', type=float, required=False, default=0.0002)
        parser.add_argument('--learning-rate-refiner', type=float, required=False, default=0)
        # Training setting
        parser.add_argument('--finetune', action='store_true', required=False, default=False)
        ...
        parser.add_argument('--seq-length-lr', type=int, required=False, default=15)
        ...
        # Tensorboard logging
        parser.add_argument('--log-dir', type=str, required=False, default='Experiments/Mamba/exp_00/tensorboard')
        parser.add_argument('--log-train-loss-interval', type=int, default=50)
        parser.add_argument('--log-train-images-interval', type=int, default=50)
        # Checkpoint loading and saving
        parser.add_argument('--checkpoint', type=str, required=False)
        parser.add_argument('--checkpoint-dir', type=str, required=False, default='Experiments/Mamba/exp_00/checkpoint')
        parser.add_argument('--checkpoint-save-interval', type=int, default=50)
        # Distributed
        parser.add_argument('--distributed-addr', type=str, default='localhost')
        parser.add_argument('--distributed-port', type=str, default='12355')  # default='12355')
        # Debugging
        parser.add_argument('--disable-progress-bar', action='store_true')
        parser.add_argument('--disable-validation', action='store_true', default=False)
        parser.add_argument('--disable-mixed-precision', action='store_true', default=False)
    ...
    def init_distributed(self, rank, world_size):
        ...
        self.dev_ids = [local_rank]
        dist.init_process_group("nccl", rank=rank, world_size=world_size)
        dist.barrier(device_ids=self.dev_ids)
    ...
    def init_model(self):
        ...
        if self.args.checkpoint:
            ...
            self.log(self.model.load_state_dict(
                torch.load(self.args.checkpoint, map_location=f'cuda:{self.rank}'), strict=False))

        self.model = nn.SyncBatchNorm.convert_sync_batchnorm(self.model)
        self.model_ddp = DDP(self.model, device_ids=[self.rank], broadcast_buffers=False, find_unused_parameters=True)

        self.optimizer = AdamW([
            {'params': self.model.stem.parameters(), 'lr': self.args.learning_rate_backbone, 'weight_decay': 10e-5, 'eps': 1e-5},
            {'params': self.model.backbone.parameters(), 'lr': self.args.learning_rate_backbone, 'weight_decay': 10e-5, 'eps': 1e-5},
            {'params': self.model.aspp.parameters(), 'lr': self.args.learning_rate_aspp, 'weight_decay': 10e-5, 'eps': 1e-5},
            {'params': self.model.decoder.parameters(), 'lr': self.args.learning_rate_decoder, 'weight_decay': 10e-5, 'eps': 1e-5},
            {'params': self.model.project_mat.parameters(), 'lr': self.args.learning_rate_decoder, 'weight_decay': 10e-5, 'eps': 1e-5},
            {'params': self.model.project_seg.parameters(), 'lr': self.args.learning_rate_decoder, 'weight_decay': 10e-5, 'eps': 1e-5},
            {'params': self.model.refiner.parameters(), 'lr': self.args.learning_rate_refiner, 'weight_decay': 10e-5, 'eps': 1e-5},
        ])

        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=self.args.epoch_end)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            if not self.args.disable_validation and (self.step % self.args.checkpoint_save_interval == 0):
                self.validate()
    ...
    def train_mat(self, true_fgr, true_pha, true_bgr, downsample_ratio, tag):
        ...
        if tag == 'hr':
            true_fgr, true_pha, true_bgr = self.final_resize(((self.args.resolution_hr // 2), self.args.resolution_hr),
                                                             true_fgr, true_pha, true_bgr)
        else:
            true_fgr, true_pha, true_bgr = self.final_resize(((self.args.resolution_lr // 2), self.args.resolution_lr),
                                                             true_fgr, true_pha, true_bgr)

        true_src = true_fgr * true_pha + true_bgr * (1 - true_pha)

        with autocast("cuda", enabled=not self.args.disable_mixed_precision):
    ...
    def train_seg(self, true_img, true_seg, log_label):
        ...
        true_img, true_seg = self.final_resize(((self.args.resolution_lr // 2), self.args.resolution_lr), true_img,
                                               true_seg)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            with autocast("cuda", enabled=not self.args.disable_mixed_precision):
                for true_fgr, true_pha, true_bgr in tqdm(self.dataloader_valid, disable=self.args.disable_progress_bar,
                                                         dynamic_ncols=True):
                    true_fgr = true_fgr.to(self.rank, non_blocking=True)
                    true_pha = true_pha.to(self.rank, non_blocking=True)
                    true_bgr = true_bgr.to(self.rank, non_blocking=True)
                    ...
                    pred_fgr, pred_pha = self.model_ddp(true_src)[:2]
        ...
        if self.rank == 0:
            ...
            self.writer.add_image(f'val_true_pha', make_grid(true_pha.flatten(0, 1), nrow=true_pha.size(1)), self.step)
            self.writer.add_image(f'val_pred_pha', make_grid(pred_pha.flatten(0, 1), nrow=pred_pha.size(1)), self.step)
            self.writer.add_image(f'val_src_pha', make_grid(true_src.flatten(0, 1), nrow=pred_pha.size(1)), self.step)
    ...
    def final_resize(self, size, *imgs):
        results = []
        for img in imgs:
            B, T = img.shape[:2]
            img = img.flatten(0, 1)  # [B*T, C, H, W]
            img = F.interpolate(img, size=size, mode='bilinear', align_corners=False)
            img = img.reshape(B, T, *img.shape[1:]).contiguous()  # restore [B, T, C, H, W]
            results.append(img)
        return results
